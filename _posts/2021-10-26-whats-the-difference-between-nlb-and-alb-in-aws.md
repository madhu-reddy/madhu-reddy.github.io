...
In the provided example, all L4 TCP requests arriving at the NLB (on port 80 or 8080) are directed to a target group (`madhu-tcp`), subsequently forwarding requests to an instance on port 80.
...
ALBs establish two separate TCP connections. The first is between the client and ALB, which is terminated by the ALB, followed by a new TCP connection from the ALB to the upstream server.

In this example, the ALB comprises three listeners:
...
In the second listener, two rules are configured. The first rule directs the ALB to forward requests to the target group (`madhu-http2`) if the URL contains a query string with the key `name` and value `ramu8080`. Otherwise, using the second rule, it forwards all requests to the target group (`madhu-http`).
...
| **Routing based on** | IP address & port | HTTP headers, cookies, query strings |
| **TCP connections** | Single connection (NAT) | Two separate connections |
| **Packet inspection** | No | Yes |
| **Speed** | Faster | Slightly slower due to inspection |
| **Use case** | High-performance, low-latency TCP/UDP traffic | HTTP/HTTPS traffic with advanced routing rules |
