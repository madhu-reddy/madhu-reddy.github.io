...
```
Src IP: client IP
Src Port: Any port in the ephemeral ports range (typically 1024-65535)
Dst IP: EC2 instance or ELB IP
Dst Port: 80
```

To ensure the response reaches the actual client, we must have an outbound rule explicitly defined:
```
Destination: 0.0.0.0/0
Port range: ephemeral ports range
```

The `ephemeral ports` range is used because the actual client's source port is dynamic and can be any port within that range.
