...
Run `git-crypt init` to set up `git-crypt` within your Git repository. This generates the encryption key and configures the repository to use `git-crypt`.
...
Define the files intended for encryption in the `.gitattributes` file by listing the file(s) and specifying `git-crypt` as the filter.
...
### 4.1 Generate a GPG key

Generate a GPG key on your local machine. This key is necessary for decrypting the repository's encrypted files.
...
Real name: madhu kunta     # Not mandatory, but it would be good if it matches the real GitHub username
Email address: madhu@mylearningsguru.com # Not mandatory, but it would be good if it matches the real GitHub user's email
Comment:
...
```bash
gpg --armor --export --output pubkeyname.gpg pubkeyID
```
...
### 4.4 Import the GPG public key on the server where git-crypt was initialized
...
## Step 5: Adding the User GPG Key to git-crypt
...
Perform a `git push` to push the auto-generated commit created after adding the user's GPG key to `git-crypt`.
...
On the client machine, clone the repository or run `git pull`.
...
Run `git-crypt unlock` and enter the passphrase used when generating the GPG key.
